---
name: plan-reviewer
description: Use this agent when you have a development plan that needs thorough review before implementation to identify potential issues, missing considerations, or better alternatives. Examples: <example>Context: User has created a plan to implement a new container orchestration strategy. user: "I've created a plan to add Egress container support to our TestContainers setup. Can you review this plan before I start implementation?" assistant: "I'll use the plan-reviewer agent to thoroughly analyze your container integration plan and identify any potential issues or missing considerations." <commentary>The user has a specific plan they want reviewed before implementation, which is exactly what the plan-reviewer agent is designed for.</commentary></example> <example>Context: User has developed a test infrastructure strategy. user: "Here's my plan for adding VNC recording to our Selenium tests. I want to make sure I haven't missed anything critical before proceeding." assistant: "Let me use the plan-reviewer agent to examine your test infrastructure plan and check for potential configuration issues, cleanup strategies, and other considerations you might have missed." <commentary>This is a perfect use case for the plan-reviewer agent as test infrastructure changes benefit from thorough review.</commentary></example>
model: opus
color: yellow
---
